...
## 8a) Here, you have to create all possible coveriance matrices.
## Level 1
vf1_1 <- varIdent(form = ~1|Cond)
vf1_2 <- varIdent(form = ~1|anotherFactor1)
vf1_3 <- varIdent(form = ~1|anotherFactor2)
## Level 2
vf2_1 <- varIdent(form = ~1|Cond * anotherFactor1)
vf2_2 <- varIdent(form = ~1|Cond * anotherFactor1)
vf2_3 <- varIdent(form = ~1|anotherFactor1 * anotherFactor2)
## Level 3
vf3 <- varIdent(form = ~1|Cond * anotherFactor1 * anotherFactor2)

## 9) Here, you create the models with:
## Default Coveriance matrix
fit1_lme0  <- lme(Y ~ Cond * anotherFactor1 * anotherFactor2, random=~1|Pp ,data = na.omit(data1), method="REML" )
## Symmetrical Coveriance Matrix 
fit1_lme1 <- lme(Y ~ Cond * anotherFactor1 * anotherFactor2, random=~1|Pp, correlation = cf1 ,data = na.omit(data1), method="REML" )
## Matrix weighted for the effects of Cond
## 9a) create ALL possible models
## Level 1
fit1_lme1_1_1  <- lme(Y ~Cond * anotherFactor1 * anotherFactor2, random=~1|Pp , weights = vf1_1   ,  data = na.omit(data1), method="REML" )
fit1_lme1_1_2  <- lme(Y ~Cond * anotherFactor1 * anotherFactor2, random=~1|Pp , weights = vf1_2   ,  data = na.omit(data1), method="REML" )
fit1_lme1_1_3  <- lme(Y ~Cond * anotherFactor1 * anotherFactor2, random=~1|Pp , weights = vf1_3   ,  data = na.omit(data1), method="REML" )
## Level 2
fit1_lme1_2_1  <- lme(Y ~Cond * anotherFactor1 * anotherFactor2, random=~1|Pp , weights = vf2_1   ,  data = na.omit(data1), method="REML" )
fit1_lme1_2_2  <- lme(Y ~Cond * anotherFactor1 * anotherFactor2, random=~1|Pp , weights = vf2_2   ,  data = na.omit(data1), method="REML" )
fit1_lme1_2_3  <- lme(Y ~Cond * anotherFactor1 * anotherFactor2, random=~1|Pp , weights = vf2_3   ,  data = na.omit(data1), method="REML" )
## Level 3
fit1_lme1_3  <- lme(Y ~Cond * anotherFactor1 * anotherFactor2, random=~1|Pp , weights = vf3   ,  data = na.omit(data1), method="REML" )
...
anova(fit1_lme0 , fit1_lme1, fit1_lme1_1_1, fit1_lme1_1_2, fit1_lme1_1_3, fit1_lme1_2_1, fit1_lme1_2_2, fit1_lme1_2_3, fit1_lme1_3)
...
fit1_lme0_ML  <- lme(Y ~  Cond * anotherFactor1 * anotherFactor2, random=~1|Pp, weights = vf1, control = lmeControl(maxIter = 100 , msMaxIter =100, niterEM = 50 ) ,data = na.omit(data1), method="ML" )
...
fit1_lme_Y   <- lme(Y ~  Cond * anotherFactor1 * anotherFactor2 , random=~1|Pp, weights = vf1, control = lmeControl(maxIter = 100 , msMaxIter =100, niterEM = 50 ),data = na.omit(data1), method="REML" )#
# 14) Create final model without weighted covariance matrix (to check its effect)
fit1_lme_Ya  <- lme(Y ~Cond * anotherFactor1 * anotherFactor2, random=~1|Pp, data = na.omit(data1), method="REML" ) #WITHOUT VARIANCE WEIGHTING OUT OF INTEREST
...
with(na.omit(data1),plot(anotherFactor1, res_fit1_lme_Y))
with(na.omit(data1),plot(2, res_fit1_lme_Y))
...
with(na.omit(data1),plot(anotherFactor1, res_fit1_lme_Ya))
with(na.omit(data1),plot(anotherFactor2, res_fit1_lme_Ya))
...
bartlett.test(  as.numeric(res_fit1_lme_Y) , dat$anotherFactor1  )
bartlett.test(  as.numeric(res_fit1_lme_Y) , dat$anotherFactor2  )
## Using 'Levene' (more reliable, particularly with large datasets)
levene.test(  as.numeric(res_fit1_lme_Y) , dat$Cond  )
levene.test(  as.numeric(res_fit1_lme_Y) , dat$anotherFactor1  )
levene.test(  as.numeric(res_fit1_lme_Y) , dat$anotherFactor2  )
...
## Repeat steps 18 - 20 for each main effect and interaction effect (only useful if the effects were significant)
## Main effects:
## Level 1
(lsm <- lsmeans(fit1_lme_Y, ~anotherFactor1, data = na.omit(data1) )) ## 18)
(temp1 <- cld(lsm))                                                   ## 19)
(temp <- contrast(lsm, method ="pairwise"))                           ## 20)

(lsm <- lsmeans(fit1_lme_Y, ~anotherFactor2, data = na.omit(data1) )) ## 18)
(temp1 <- cld(lsm))                                                   ## 19)
(temp <- contrast(lsm, method ="pairwise"))                           ## 20)

## Interaction effects:
## Level 2
(lsm <- lsmeans(fit1_lme_Y, ~Cond1 * anotherFactor1, data = na.omit(data1) )) ## 18)
(temp1 <- cld(lsm))                                                   ## 19)
(temp <- contrast(lsm, method ="pairwise"))                           ## 20)

(lsm <- lsmeans(fit1_lme_Y, ~Cond1 * anotherFactor2, data = na.omit(data1) )) ## 18)
(temp1 <- cld(lsm))                                                   ## 19)
(temp <- contrast(lsm, method ="pairwise"))                           ## 20)

(lsm <- lsmeans(fit1_lme_Y, ~anotherFactor1 * anotherFactor2, data = na.omit(data1) )) ## 18)
(temp1 <- cld(lsm))                                                   ## 19)
(temp <- contrast(lsm, method ="pairwise"))                           ## 20)

## Level 3
(lsm <- lsmeans(fit1_lme_Y, ~Cond * anotherFactor1 * anotherFactor2, data = na.omit(data1) )) ## 18)
(temp1 <- cld(lsm))                                                   ## 19)
(temp <- contrast(lsm, method ="pairwise"))                           ## 20)
